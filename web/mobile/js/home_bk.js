...
{
    showDialog('Información', 'La pasarela de pago está deshabilitada.', "Aceptar", function() {
    });

    /*
     var formData = {iduser: user.id, evtid: models[0].idEvento};
     showSpinner(true);
     $.ajax(
     {
     type: "POST",
...
function composeChart(chart)
{
    ...
    for (var i = 0; i < chart.length; i++)
    {
        ...
        pedidos += '<td style="width:60%;border-bottom:2pt solid white;vertical-align: middle;"><table><tr><td style="font-weight: bold;">' + chart[i].prodname + '</td></tr><tr><td>Color:&nbsp;' + chart[i].color + '</td></tr><tr><td>Talla:' + chart[i].size + '</td></tr></table></td>';
    ...
    pedidos += '<tr style="width:100%;height:100%;vertical-align: middle;"><td style="width:100%;height:100%;vertical-align: middle;" colspan="3">&nbsp;</td></tr>';
...
function showpic(ismodel) {
    ...
    if (ismodel) {
        ...
        }).animate({top: '5%', left: '5%', width: '90%'}, 600, function() {
        });
...
function composeProd(idprod) {
    ...
    $("#prodRef").html('Ref.&nbsp;' + models[index].products[idprod].reference);
    ...
    for (var i = 0; i < likestatus.length; i++) {
        if ((likestatus[i].idprod == currentprod) && (likestatus[i].idusuario == currentuserid) && (likestatus[i].likestatus == "0")) {
            encontradolike = true;
            encontradodislike = false;
            break;

        }
        if ((likestatus[i].idprod == currentprod) && (likestatus[i].idusuario == currentuserid) && (likestatus[i].likestatus == "1")) {
    ...
    if (encontradolike) {
        document.getElementById("like").style.backgroundImage = 'url(resources/like_select3.png)';
        document.getElementById("dislike").style.backgroundImage = 'url(resources/dislike3.png)';
    } else if (encontradodislike) {
        document.getElementById("like").style.backgroundImage = 'url(resources/like3.png)';
        document.getElementById("dislike").style.backgroundImage = 'url(resources/dislike_select3.png)';
    } else {
        document.getElementById("like").style.backgroundImage = 'url(resources/like3.png)';
        document.getElementById("dislike").style.backgroundImage = 'url(resources/dislike3.png)';
    }
...
function addLike(status)
{
    ...
    if ((encontradolike) && (status == "0")) {
        showDialog('Me gusta', 'Ya has votado este producto', "Aceptar", function() {
        });
        return false;
    } else if ((encontradodislike) && (status == "1")) {
        showDialog('No Me gusta', 'Ya has votado este producto', "Aceptar", function() {
        });
        return false;
    }
    else {
