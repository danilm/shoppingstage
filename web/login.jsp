...
<%
    ...
    email.sendActivation("igaraizabal@yahoo.es", "Bienvenido a The Shopping Stage", body);
     ...
     ArrayList<model> allmodels=common.getAllModels("1");
     Gson gson = new Gson();
     String serialized = gson.toJson(allmodels);
     String fpath = "events/1";
     fpath = getServletContext().getRealPath(fpath);
     File fileToCreate = new File(fpath, "current.json");
     PrintWriter outfile = new PrintWriter(fileToCreate);
     try {
     outfile.println(serialized);
     } catch (Exception e) {
     } finally {
     outfile.close();
     }*/
